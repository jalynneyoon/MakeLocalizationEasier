...
previous_list = list(map(str, data['mk']))
current_list = list(map(str, data['nk']))
directory_path = "/Users/yunjohyeon/ssm-mobile-ios/SinsangMarket_Dome"
...
def change_key(path):
    ...
    for idx, line in enumerate(fp.readlines()):
        ...
        for idx in range(0, len(previous_list)):
            if '\"' + previous_list[idx] + '\".localized(hint:' in line:
                print("** 중복 값 있음: ", previous_list[idx])
                new_line = new_line.replace('\"' + previous_list[idx] + '\".localized(hint:', '\"' + current_list[idx] + '\".localized(hint:')
